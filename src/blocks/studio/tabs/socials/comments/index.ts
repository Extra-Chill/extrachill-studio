...
const CommentsView = ( { slug, label }: CommentsViewProps ): ReactElement => {
	...
	const [ replyFeedback, setReplyFeedback ] =
		useState< ReplyFeedback | null >( null );
	...
	const loadComments = useCallback(
		async ( mediaId: string = '' ): Promise< void > => {
			...
			try {
				...
				if ( loaded.length === 0 ) {
					setStatus(
						__( 'No comments found.', 'extrachill-studio' )
					);
				} else {
					setStatus(
						sprintf(
							/* translators: %d: loaded comment count. */
							__( 'Loaded %d comments.', 'extrachill-studio' ),
							loaded.length
						)
					);
				}
				...
				setError(
					( fetchError as Error )?.message ||
						__( 'Unable to load comments.', 'extrachill-studio' )
				);
			} finally {
				setIsLoading( false );
			}
	...
	let filterButtonLabel: string = __( 'Refresh All', 'extrachill-studio' );
	if ( isLoading ) {
		filterButtonLabel = __( 'Loading…', 'extrachill-studio' );
	} else if ( postFilter.trim() ) {
		filterButtonLabel = __( 'Filter Comments', 'extrachill-studio' );
	}
	...
	return h(
		...
		h(
			...
			h( PanelHeader, {
				description: sprintf(
					...
					__(
						'Comments on your %s posts. Filter by post ID or view all recent comments.',
						'extrachill-studio'
					),
					platformLabel
				),
			...
			h(
				...
				h(
					FieldGroupView,
					{
						...
						help: __(
							'Enter a post/media ID to filter, or leave empty to show all recent comments.',
							'extrachill-studio'
						),
					},
					createElement( 'input', {
		...
		replyFeedback
			? createElement(
					...
					{
						role:
							replyFeedback.tone === 'error' ? 'alert' : 'status',
						'aria-live':
							replyFeedback.tone === 'error'
								? 'assertive'
								: 'polite',
						'aria-atomic': true,
					},
		...
		comments.length > 0
					...
					h(
						...
						...comments.map( ( comment ) =>
							createElement(
								...
								{
									key: comment.id,
									className: 'ec-studio-comment-list__item',
								},
								createElement(
									'div',
									{
			...
			: ! isLoading &&
					h(
						...
						createElement(
							'div',
							{ className: 'ec-studio-preview' },
							__( 'No comments to display.', 'extrachill-studio' )
						)
					)
	);
};
